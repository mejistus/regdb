...
echo "PAPER_FULL_PUBLISH_WATCH_START:$(date -Is)"
echo "ROOT:$ROOT TRAIN_LOG:$TRAIN_LOG TRIALS:$TRIALS"

while ! grep -q "PAPER_AMP_FULL_ABLATION_DONE" "$TRAIN_LOG" 2>/dev/null; do
  echo "WAITING_PAPER_AMP_FULL:$(date -Is)"
  sleep "$POLL_SECONDS"
done

echo "PAPER_AMP_FULL_DONE_DETECTED:$(date -Is)"
...
echo "PAPER_FULL_PUBLISH_DONE:$(date -Is)"
